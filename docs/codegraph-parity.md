...
| Framework resolvers | Partial | Express, FastAPI, React Router JSX/object routes, Django `path`/`re_path`/legacy `url`, DRF viewsets, NestJS HTTP controllers plus import-proven websocket, message/event pattern, and unary/streaming gRPC endpoints, and Vue/Svelte template children and events; adversarial fixtures and pinned real-repository gates | GraphQL, broader Vue/Svelte directives, prioritized remaining adapters, and a second real repository per adapter |
