...
| Callbacks and dynamic dispatch | Partial | named callbacks; exact byte-correlated direct formal-to-actual propagation and bounded formal-to-formal delegation; accepted JS-family/ArkTS inline closure identities with nested ownership and rejected-call fallback; immediate TS/TSX/ArkTS call-result receivers through explicit simple nominal return annotations; initializer/object/array/assignment references; strict import/local/Harmony-project resolution; literal events; React `setState → render → JSX child`; bounded interface-to-implementation dispatch; app-scoped Harmony emitter channels with named/inline callbacks, immutable imported descriptors/member constants, bounded barrel forwarding, and real OpenHarmony direct/delegated/call-result evidence; fanout/depth/work caps, incremental cleanup, rollback, provenance, adversarial tests, and pinned differential checks | inferred/generic/union/Promise return flow, deeper call-result chains, typed arrow factories, constructor-built descriptors, inline callable forms in remaining languages, broader member/type dispatch, inherited implementations, Flutter rebuilds, and remaining cross-language event channels |
