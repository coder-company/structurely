...
| Core CLI/MCP names and schemas | Compatible (core) | eight handlers, pinned contract fixture, persistent-stdio binary test, and 16-scenario live differential run against pinned CodeGraph | pagination and cross-platform suites |
...
| Callbacks and dynamic dispatch | Partial | named callbacks; initializer/object/array/assignment references; strict import/local resolution; literal events; React `setState → render → JSX child`; fanout caps, incremental cleanup, provenance, adversarial tests, and pinned differential checks | broader member/type dispatch, Flutter rebuilds, interface implementations, and cross-language event channels |
