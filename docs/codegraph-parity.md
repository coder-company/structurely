...
| Callbacks and dynamic dispatch | Partial | named callbacks; exact byte-correlated direct formal-to-actual propagation and bounded formal-to-formal delegation; initializer/object/array/assignment references; strict import/local/Harmony-project resolution; literal events; React `setState → render → JSX child`; bounded interface-to-implementation dispatch; app-scoped Harmony emitter channels with named/inline callbacks, immutable imported descriptors/member constants, bounded barrel forwarding, and real KeyManager evidence; fanout/depth/work caps, incremental cleanup, rollback, provenance, adversarial tests, and pinned differential checks | inline-closure argument identities, constructor-built descriptors, broader member/type dispatch, inherited implementations, Flutter rebuilds, and remaining cross-language event channels |
